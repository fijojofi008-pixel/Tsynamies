...
for i in 1:N  
    for j in eachindex(nodes)                                                                                   u[i, j] = f(to_physical_element(x_l, x_r, nodes[j]))  
    end  
...
return u                                                                                            endfunction rk3_step(u, dt, rhs)  
u1 = u + dt * rhs(u)  
